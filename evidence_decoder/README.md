...
|---|---|---|---|---|---|---|
| raw | 1.00 | 0.00 | **0.20** | 1.00 | 0.92 | 1178ms |
| 1층만 | 0.96 | **1.00** | **1.00** | 1.00 | 0.92 | 10712ms |
| 1층+2층 | 0.95 | **1.00** | **1.00** | 1.00 | 0.91 | 12838ms |

합성 실험의 세 결론이 모두 재현되었다.
1. 근거 희석 억제는 1층의 기여다 (근거정밀 0.20 -> 1.00).
2. 1층은 실제 검색기가 가져온 distractor 도 전부 걸러낸다 (무관거부 1.00).
3. 답변 요지는 잡음에 강건하다 (요지충족이 세 구성 모두 0.91~0.92).

대가는 gold 채택률이 1.00 -> 0.95~0.96 으로 조금 깎이는 것과 지연이다.

**중복·충돌 처리 - 2층의 기여**

쌍 판정 도입 후 (시나리오 3종 x 3회 = 행당 n=9)

| 패킷 유형 | 중복인지 | 중복제거 | 충돌탐지 | 근거정밀 | 인용정밀 |
|---|---|---|---|---|---|
| 중복 | 0.78 | 0.78 | - | 0.94 | 0.94 |
| 충돌 | - | - | 1.00 | 0.94 | 0.97 |
| 중복+충돌+잡음 | 1.00 | 1.00 | 0.89 | 0.73 | 0.75 |

통합 계층이 없으면 중복인지·충돌탐지·중복제거가 모두 0.00 이다.
...
| 중복 인지·제거 (쌍 판정) | 중간 | 0.00 -> 0.78~1.00, 시나리오 편차 해소, n=9 |
